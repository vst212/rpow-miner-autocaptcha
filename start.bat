...
rem 2. Load .env via Node.js (avoids for/f quoting issues with :: in blocks)
set "CAPTCHA_KEY="
node -e "try{var l=require('fs').readFileSync('.env','utf8').split(/\r?\n/);for(var i=0;i<l.length;i++){var t=l[i].trim();if(t.indexOf('YESCAPTCHA_KEY=')===0){console.log(t.slice(15).replace(/^['\x22]|['\x22]$/g,''));break}}}catch(e){}" > _tmp_env.txt 2>nul
...
rem 3. Load saved config (resume detection)
set "SAVED_EMAIL="
set "SAVED_ENGINE="
set "SAVED_WORKERS="
set "SAVED_GPU_BATCH="
